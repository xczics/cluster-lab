...
Phase 1（集群部署）完成后，集群能正常启动、节点注册正常，但 MPI 只能跑单节点。HPC 集群的核心能力是跨节点并行计算——没有跨节点通信，计算集群无法发挥分布式算力优势。因此需要打通跨节点 MPI 通道。
...
Ubuntu apt 的 Slurm 包已含 PMIx 支持，**不需要手动从源码编译 Slurm 了**，节省了一个大麻烦（编译 Slurm 依赖链很深）。
